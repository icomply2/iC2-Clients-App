...
const INSURANCE_COVER_OPTIONS = ["Life", "TPD", "Trauma", "Income Protection", "Health", "Other"];
...
function getPopupFields(section: FactFindPopupSection, ownerOptions: Array<{ label: string; value: string }>): PopupField[] {
  ...
  switch (section) {
    ...
    case "insurance":
      return [
        ownerField,
        { key: "coverRequired", label: "Cover", input: "select", options: options(INSURANCE_COVER_OPTIONS) },
        { key: "insurer", label: "Insurer", input: "text" },
        { key: "sumInsured", label: "Sum Insured", input: "currency" },
        { key: "premiumAmount", label: "Premium", input: "currency" },
        frequencyField,
        { key: "status", label: "Status", input: "select", options: options(INSURANCE_STATUS_OPTIONS) },
        jointField,
      ];
  }
}
...
export function FactFindSection({ clientId, profile }: FactFindSectionProps) {
  ...
  const recordModalFields = recordModal ? getPopupFields(recordModal.section, ownerOptions) : [];
