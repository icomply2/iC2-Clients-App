...
export async function POST(request: NextRequest, context: { params: Promise<{ licenseeId: string }> }) {
  ...
  if (!Array.isArray(payload)) {
    return NextResponse.json({ message: "Asset class array is required." }, { status: 400 });
  }

  const { licenseeId } = await context.params;
  const response = await fetch(new URL(`/api/Licensees/${encodeURIComponent(licenseeId)}/AssetClasses`, API_BASE_URL), {
    method: "POST",
    headers: {
