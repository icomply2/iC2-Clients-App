...
import { API_BASE_URL, readBearerToken, requireCurrentUser } from "../../client-profiles/_shared";
...
export async function POST(request: NextRequest) {
  ...
  try {
    const response = await fetch(new URL("/api/PlatformIntegrations/Mapping", API_BASE_URL), {
      method: "POST",
      headers: {
      ...
      body: JSON.stringify({
        ...requestBody,
        linkedByUserId: currentUserResult.currentUser.id ?? "",
      }),
      cache: "no-store",
    });
    ...
    return NextResponse.json(
      {
        message:
          error instanceof Error
            ? `Platform integration mapping create proxy failed: ${error.message}`
            : "Platform integration mapping create proxy failed.",
      },
      { status: 502 },
    );
  }
