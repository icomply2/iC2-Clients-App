...
export async function GET(request: NextRequest, { params }: { params: Promise<{ accountId: string }> }) {
  ...
  try {
    const response = await fetch(
      new URL(`/api/ClientProfiles/Account/${encodeURIComponent(accountId)}/Portfolio`, API_BASE_URL),
      {
        method: "GET",
        headers: {
    ...
    return NextResponse.json(
      {
        message:
          error instanceof Error ? `Portfolio holdings load proxy failed: ${error.message}` : "Portfolio holdings load proxy failed.",
      },
      { status: 502 },
    );
  }
...
export async function POST(request: NextRequest, { params }: { params: Promise<{ accountId: string }> }) {
  ...
  try {
    ...
    return NextResponse.json(
      {
        message:
          error instanceof Error ? `Portfolio holding create proxy failed: ${error.message}` : "Portfolio holding create proxy failed.",
      },
      { status: 502 },
    );
  }
