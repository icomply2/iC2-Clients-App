...
async function forwardPersonUpdate(
  ...
  try {
    const response = await fetch(
      new URL(`/api/ClientProfiles/${profileId}/Partner/${personId}`, API_BASE_URL),
      {
        method,
        headers: {
    ...
    const message =
      error instanceof Error ? `Partner update proxy failed: ${error.message}` : "Partner update proxy failed.";
