...
export async function POST(
  ...
  try {
    const response = await fetch(new URL(`/api/ClientProfiles/${profileId}/Assets`, API_BASE_URL), {
      method: "POST",
      headers: {
        Accept: "application/json",
    ...
    const message =
      error instanceof Error ? `Asset save proxy failed: ${error.message}` : "Asset save proxy failed.";
