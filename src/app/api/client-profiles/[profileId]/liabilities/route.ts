...
export async function POST(request: NextRequest, { params }: { params: Promise<{ profileId: string }> }) {
  ...
  try {
    const response = await fetch(new URL(`/api/ClientProfiles/${encodeURIComponent(profileId)}/Liabilities`, API_BASE_URL), {
      method: "POST",
    ...
    return NextResponse.json({ message: error instanceof Error ? `Liability save proxy failed: ${error.message}` : "Liability save proxy failed." }, { status: 502 });
  }
}
