...
export async function POST(request: NextRequest, { params }: { params: Promise<{ profileId: string }> }) {
  ...
  try {
    const response = await fetch(new URL(`/api/ClientProfiles/${encodeURIComponent(profileId)}/Incomes`, API_BASE_URL), {
      method: "POST",
    ...
    return NextResponse.json({ message: error instanceof Error ? `Income save proxy failed: ${error.message}` : "Income save proxy failed." }, { status: 502 });
  }
}
