...
export async function POST(
  ...
  try {
    const response = await fetch(
      new URL(`/api/ClientProfiles/${profileId}/Dependants`, API_BASE_URL),
      {
        method: "POST",
        headers: {
          Accept: "application/json",
    ...
    const message =
      error instanceof Error ? `Dependant save proxy failed: ${error.message}` : "Dependant save proxy failed.";
