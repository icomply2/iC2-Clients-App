...
export async function POST(request: NextRequest, { params }: { params: Promise<{ profileId: string }> }) {
  ...
  try {
    const response = await fetch(new URL(`/api/ClientProfiles/${encodeURIComponent(profileId)}/SuperAnnuations`, API_BASE_URL), {
      method: "POST",
    ...
    return NextResponse.json({ message: error instanceof Error ? `Superannuation save proxy failed: ${error.message}` : "Superannuation save proxy failed." }, { status: 502 });
  }
}
