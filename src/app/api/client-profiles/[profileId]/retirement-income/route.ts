...
export async function POST(request: NextRequest, { params }: { params: Promise<{ profileId: string }> }) {
  ...
  try {
    const response = await fetch(new URL(`/api/ClientProfiles/${encodeURIComponent(profileId)}/Pensions`, API_BASE_URL), {
      method: "POST",
    ...
    return NextResponse.json({ message: error instanceof Error ? `Retirement income save proxy failed: ${error.message}` : "Retirement income save proxy failed." }, { status: 502 });
  }
}
