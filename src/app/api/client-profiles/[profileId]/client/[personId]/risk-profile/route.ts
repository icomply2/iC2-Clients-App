...
export async function PUT(
  ...
  try {
    const response = await fetch(
      new URL(`/api/ClientProfiles/${encodeURIComponent(profileId)}/Client/${encodeURIComponent(personId)}/RiskProfile`, API_BASE_URL),
      {
        method: "PUT",
        headers: {
    ...
    return NextResponse.json(
      { message: error instanceof Error ? `Client risk profile proxy failed: ${error.message}` : "Client risk profile proxy failed." },
      { status: 502 },
    );
  }
}
