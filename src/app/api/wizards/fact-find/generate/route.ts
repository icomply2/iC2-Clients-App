...
async function loadProfile(clientId: string) {
  ...
  if (!token) {
    throw new Error("You need to sign in again before generating the fact find document.");
  }
  ...
  if (!profileIdResult.data) {
    throw new Error("Finley could not resolve the client profile needed for document generation.");
  }
...
export async function POST(request: NextRequest) {
  const body = (await request.json().catch(() => null)) as { clientId?: string | null } | null;
  ...
  try {
    ...
    const message =
      error instanceof ApiError
        ? `Unable to load the live fact find data (${error.statusCode}): ${error.message}`
